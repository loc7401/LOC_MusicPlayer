...
function AudioPlayer({ currentTrack, currentIndex, setCurrentIndex, total, album }) {
    ...
    return (
        <div className="flex md:w-4/6 md:m-0 mb-2.5 w-full flex-col justify-betwee items-center bg-[#8d8d9159] rounded-[20px]">
            ...
            <div className="p-2 md:p-5 w-full flex flex-col items-center">
                <p className="text-[#9aa9c2] text-base font-medium px-5 text-center mt-1 overflow-hidden text-ellipsis text-nowrap w-full">{`${
                    album?.name
                } - ${artist.join(", ")}`}</p>
                <Controls
                    isPlaying={isPlaying}
